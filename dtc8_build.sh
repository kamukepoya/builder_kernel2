...
# Kernel Sources
     ...
     git clone --depth=1 https://github.com/NusantaraDevs/DragonTC -b 8.0 clang
...
function push() {
    ...
    curl -F document=@$ZIP "https://api.telegram.org/bot$TG_TOKEN/sendDocument" \
        ...
        -F "parse_mode=html" \
        -F caption="Compile took $(($DIFF / 60)) minute(s) and $(($DIFF % 60)) second(s). | For <b>$DEVICE_CODENAME</b> | <b>DTC8</b>"
}
# Fin Error
function finerr() {
...
function zipping() {
    cd AnyKernel || exit 1
    zip -r9 $KERNELNAME-[DTC8]-$DATE.zip *
    cd ..
}
compile
zipping
